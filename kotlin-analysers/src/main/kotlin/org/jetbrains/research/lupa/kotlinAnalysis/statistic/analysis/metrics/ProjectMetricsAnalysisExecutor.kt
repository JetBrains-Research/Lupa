...
/**
 * Executor for project metrics analysis which collects number of files,
...
class ProjectMetricsAnalysisExecutor(
    ...
    private val projectMetricsDataWriter = PrintWriterResourceManager(
        outputDir, filename,
        listOf("project_name", "files_count", "lines_count", "symbols_count")
            .joinToString(separator = ",")
    )

    override val controlledResourceManagers: Set<ResourceManager> = setOf(projectMetricsDataWriter)

    override val requiredFileExtensions: Set<FileExtension> = setOf(FileExtension.KT)

    override fun analyse(project: Project) {
        val documentManager = PsiDocumentManager.getInstance(project)
        val files = project.extractPsiFiles()
