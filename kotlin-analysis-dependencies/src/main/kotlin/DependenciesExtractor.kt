...
class DependenciesExtractor {
    ...
    fun extractDependencies(inputDir: Path, outputDir: Path) {
        var nWithLibraries = 0
        var n = 0
        val fileWriterWith = getPrintWriter(outputDir, "projects_with.csv")
        val fileWriterWithout = getPrintWriter(outputDir, "projects_without.csv")
        try {
            getSubdirectories(inputDir).forEach { projectPath ->
                ApplicationManager.getApplication().runReadAction {
                    ...
                    try {
                        ...
                        if (flag) {
                            fileWriterWith.println(project!!.name)
                            println(project!!.name)
                            nWithLibraries++
                        } else {
                            fileWriterWithout.println(project!!.name)
                            println(project!!.name)
                        }
            ...
            fileWriterWith.close()
            fileWriterWithout.close()
        }
    }
